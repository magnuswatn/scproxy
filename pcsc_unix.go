...
func (c *scContext) Connect(reader string) (*scHandle, error) {
	...
	rc := C.SCardConnect(c.ctx, C.CString(reader),
		C.SCARD_SHARE_EXCLUSIVE, C.SCARD_PROTOCOL_T0,
...
func (h *scHandle) Close() error {
	return scCheck(C.SCardDisconnect(h.h, C.SCARD_RESET_CARD))
}
...
func (t *scTx) Close() error {
	return scCheck(C.SCardEndTransaction(t.h, C.SCARD_RESET_CARD))
}

// CHANGED for scproxy: include SW2 (`more byte`) in the response, when the smart card
// has more data to send + use T0 instead of T1
